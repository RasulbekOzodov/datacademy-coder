...
export function closestMatch(content, old) {
 ...
 */
export function applyEdit(content, oldRaw, neuRaw, replaceAll = false) {
    const old = oldRaw.replace(/\r\n/g, '\n');
    const neu = neuRaw.replace(/\r\n/g, '\n');
    if (!old)
        return { ok: false, error: '"old" must not be empty. To create a file use write_file.' };
    if (old === neu)
        return { ok: false, error: '"old" and "new" are identical; nothing to change.' };
    // 1. Exact.
    ...
    const trimAll = (s) => s.trim();
    const matchesAt = (i, f) => {
        for (let k = 0; k < win; k++) {
            const l = lines[i + k];
            if (l === undefined || f(l) !== f(oldLines[k]))
                return false;
        }
        return true;
    };
    for (const [level, f] of [
        ...
        if (hits.length === 1) {
            ...
            if (level === 'indent') {
                ...
                const fileIndentOf = (k) => leadingWs(fileWin[Math.min(k, win - 1)]);
                const oldIndentOf = (k) => leadingWs(oldLines[Math.min(k, win - 1)]);
                newLines = newLines.map((l, j) => {
                    ...
                    if (exactK >= 0)
                        return fileIndentOf(exactK) + body;
...
export function makeDiff(rel, before, after, maxLines = 120) {
...
export const editFileTool = {
    name: 'edit_file',
    description: 'Replace an exact snippet of text in a file. "old" must match the file content exactly (copy it from read_file output, without the line-number prefix) and must be unique. Read the file first.',
    permission: 'write',
    parameters: {
        type: 'object',
        properties: {
    ...
    async execute(args, ctx) {
        ...
        const res = applyEdit(lf, old, neu, replaceAll);
        if (!res.ok)
            return { content: `edit_file failed for ${rel}: ${res.error}`, isError: true };
        const out = bom + (eol === '\r\n' ? res.content.replace(/\n/g, '\r\n') : res.content);

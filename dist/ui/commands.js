...
export async function pickSession(cwd, ask, print) {
...
export function applySession(ctx, id) {
    ...
    switch (cmd.toLowerCase()) {
        ...
        case 'models': {
            try {
                ...
                if (!models.length)
                    print(pc.dim('no models found on the server'));
                else
                    print(models.map((m) => (m === agent.provider.model ? `${pc.green('*')} ${m}` : `  ${m}`)).join('\n'));
            }
            catch (err) {
                print(pc.red(err.message));
            }
        ...
        case 'sessions': {
            ...
            if (!list.length)
                print(pc.dim('no saved sessions'));
            else
                print(list.map((s) => `  ${s.id}  ${pc.dim(`${s.messages} msgs · ${s.model} · ${s.updatedAt.slice(0, 16).replace('T', ' ')}`)}`).join('\n'));
            return 'handled';
        }
        default:

...
async function main(): Promise<void> {
  ...
  if (shellFallbackNote) out(`${pc.yellow(`⚠ ${shellFallbackNote}`)}\n`);
  ...
  if (debug.enabled) out(`${pc.dim(`debug log: ${debug.file}`)}\n`);
  ...
  if (opts.continue) {
    ...
    if (!resume) out(`${pc.dim('no previous conversation in this folder — starting a new one')}\n`);
...
main().catch((err: Error) => {
  ...
  if (opts.debug && err.stack) out(`${pc.dim(err.stack)}\n`);
  process.exit(1);
});

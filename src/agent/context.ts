import type { Message, ToolDef, Usage } from '../llm/types.js';
...
export class ContextManager {
  ...
  constructor(private getWindow: () => number) {}
  ...
  estimate(messages: Message[], tools?: ToolDef[], systemPrompt?: string): number {
  ...
  calibrate(promptTokens: number | undefined, estimatedTokens: number): void {
    if (!promptTokens || promptTokens < 200 || estimatedTokens < 50) return;
   ...
   */
  ensureBudget(messages: Message[], tools: ToolDef[] | undefined, systemPrompt: string): string[] {
    const notes: string[] = [];
    const window = this.getWindow();
    const budget = window * (1 - RESERVE_FOR_RESPONSE);
    let est = this.estimate(messages, tools, systemPrompt);
    if (est <= window * STUB_AT) return notes;
    ...
    if (est <= window * TRIM_AT) return notes;
  ...
  compact(messages: Message[], tools: ToolDef[] | undefined, systemPrompt: string): string {
  ...
  private stubOldToolResults(messages: Message[], keep = KEEP_FULL_TOOL_RESULTS): number {
   ...
   */
  private dropOldestTurns(messages: Message[], fits: () => boolean): number {
    let dropped = 0;
    const isUserTurn = (m: Message) => m.role === 'user' && m.meta?.kind !== 'tool_result' && m.meta?.kind !== 'parse_retry' && m.meta?.kind !== 'context';
    while (!fits() && messages.length > 2) {
      ...
      if (start >= lastUserIdx) break;
      let end = start + 1;
      while (end < messages.length && !isUserTurn(messages[end])) end++;
      if (end > lastUserIdx) break;
  ...
  private lowCountStreak = 0;

  checkUsage(usage: Usage | undefined, estimatedPrompt: number): string | undefined {
    if (!usage?.promptTokens) return undefined;
    const window = this.getWindow();
    const total = usage.promptTokens + (usage.completionTokens ?? 0);
    if (total >= window * 0.98) return `context window nearly full (${total}/${window} tokens). Use /compact or /clear.`;

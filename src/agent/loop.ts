...
import type { Config, ToolMode } from '../config/schema.js';
...
import { ToolsUnsupportedError, type AssistantTurn, type LLMProvider, type Message, type ToolCall, type Usage } from '../llm/types.js';
...
export interface AgentUI {
  ...
  onResponseEnd(): void;
  onToolStart(call: ToolCall, description: string): void;
...
type ResolvedToolMode = 'native' | 'text';

interface TurnResult extends AssistantTurn {
  rawText: string;
  parseErrors: string[];
}
 ...
 */
function missingReferencedFiles(answer: string, cwd: string): string[] {
  if (!/\b(create|creating|write|writing|add|adding|make|making|generate|save|yarat|yoz|qo'sh)\b/i.test(answer)) return [];
  ...
  for (const m of answer.matchAll(/(?<![\w/.:-])([\w][\w.-]*\/)*[\w][\w-]*\.(html|css|js|mjs|cjs|ts|tsx|jsx|py|json|md|txt|yml|yaml|toml|env|sh|ps1|java|go|rs|c|cpp|h)\b/gi)) {
    const name = m[0];
    if (/^https?:/i.test(name) || name.length > 80) continue;
    names.add(name);
  }
  const missing: string[] = [];
  for (const name of names) {
    try {
      if (!fs.existsSync(path.resolve(cwd, name))) missing.push(name);
    } catch {
      /* ignore odd names */
    }
  }
 ...
 */
function looksLikePlanOnly(answer: string, userInput: string): boolean {
  const a = answer.trim();
  if (a.length < 40) return false;
  const q = userInput.trim();
  if (/\?\s*$/.test(q)) return false; // plain questions may legitimately be answered in text
...
function repeatNudge(userInput: string): string {
...
function extractMessage(args: Record<string, unknown>): string | undefined {
  for (const k of ['message', 'text', 'content', 'answer', 'summary', 'response', 'result', 'output']) {
    const v = args[k];
    if (typeof v === 'string' && v.trim()) return v.trim();
  }
  const strings = Object.values(args).filter((v): v is string => typeof v === 'string' && v.trim().length > 0);
...
export class Agent {
  ...
  private resolvedMode?: ResolvedToolMode;
  private systemPromptCache?: { key: string; prompt: string };
  private makeId = defaultIdGen();
  ...
  get toolMode(): ResolvedToolMode | undefined {
  ...
  async resolveToolMode(): Promise<ResolvedToolMode> {
    if (this.resolvedMode) return this.resolvedMode;
    const configured: ToolMode = this.config.toolMode;
    if (configured === 'native' || configured === 'text') {
      ...
      if (!caps.nativeTools) this.opts.ui.onInfo(`model has no native tool support — using text tool-calling`);
    }
    return this.resolvedMode;
  }

  private toolDefsFor(mode: ResolvedToolMode) {
    return mode === 'native' ? this.opts.tools.defs() : undefined;
  }

  private systemPrompt(mode: ResolvedToolMode): string {
    const key = `${mode}:${this.provider.name}:${this.provider.model}`;
    if (this.systemPromptCache?.key === key) return this.systemPromptCache.prompt;
  ...
  async run(userInput: string, signal: AbortSignal): Promise<void> {
    ...
    while (iterations < this.config.maxIterations) {
      ...
      for (const note of this.context.ensureBudget(this.messages, toolDefs, systemPrompt)) ui.onInfo(note);
      ...
      if (!call) {
        ...
        if (turn.text.trim()) this.lastFinalText = turn.text;
        ...
        if (!turn.text.trim() && turn.finishReason === 'length') ui.onWarn('the response was cut off by the max token limit');
        break; // final answer
      }
      parseRetries = 0;
      ...
      for (let i = 0; i < calls.length; i++) {
        ...
        if (result.isError) {
          ...
          if (i < calls.length - 1) skipReason = `the previous call (${c.name}) failed; ${calls.length - 1 - i} later call(s) were skipped. Fix the problem, then call them again.`;
      ...
      if (stopRun || signal.aborted) break;
    }
    if (iterations >= this.config.maxIterations) ui.onWarn(`stopped after ${iterations} iterations (maxIterations). Ask me to continue if needed.`);
  }

  /** True when a new final answer is (nearly) identical to the previous run's final answer. */
  private isParroting(text: string): boolean {
    if (!this.lastFinalText) return false;
  ...
  private recordInterrupt(partialText: string): void {
    const text = `${partialText}${partialText ? '\n' : ''}[Interrupted by user]`;
    this.messages.push({ role: 'assistant', content: text, meta: { kind: 'interrupt' } });
  }

  private toolResultMessage(mode: ResolvedToolMode, call: ToolCall, content: string): Message {
    if (mode === 'native') {
  ...
  private async executeCall(call: ToolCall, signal: AbortSignal): Promise<ToolResult> {
    const ui = this.opts.ui;
    const tool = this.opts.tools.get(call.name);
    if (!tool) {
      const res: ToolResult = { content: `Unknown tool "${call.name}". Available tools: ${this.opts.tools.names().join(', ')}.`, isError: true };
    ...
    if (signal.aborted && !result.isError) result = { ...result, content: `${result.content}\n[aborted by user]` };
  ...
  private async streamTurn(mode: ResolvedToolMode, systemPrompt: string, signal: AbortSignal): Promise<TurnResult> {
    const ui = this.opts.ui;
    const messages: Message[] = [{ role: 'system', content: systemPrompt }, ...this.messages];
    const toolDefs = this.toolDefsFor(mode);
    ...
    try {
      ...
      show(gate.flush());
      if (!firstToken) ui.onFirstToken();
      if (shownAny) ui.onResponseEnd();
      if (cutByGuard) ui.onWarn('the model started repeating itself — generation cut short');
    }

    // The stop sequence eats the closing tag; restore it so history stays well-formed.
    if (rawText.includes(TOOL_CALL_OPEN) && !rawText.includes(TOOL_CALL_CLOSE)) rawText += `\n${TOOL_CALL_CLOSE}`;
...
export class RepetitionGuard {
  ...
  push(delta: string): boolean {
    ...
    while ((idx = this.buf.indexOf('\n')) >= 0) {
      ...
      if (line === this.lastLine) this.repeats++;
      else {
        this.lastLine = line;
        this.repeats = 1;
      }
      ...
      if (this.repeats >= this.maxConsecutive || total >= this.maxTotal) return true;
    }
    return false;
  }
}

  ...
  async execute(args: ToolArgs, ctx: ToolContext): Promise<ToolResult> {
    ...
    const walk = async (dir: string, level: number, prefix: string) => {
      ...
      try {
        ...
        if (code === 'ENOENT') throw new Error(`Directory not found: ${relPath(ctx.cwd, dir)}. Paths are relative to the working directory — use "." for it.`);
        if (code === 'ENOTDIR') throw new Error(`${relPath(ctx.cwd, dir)} is a file, not a directory. Use read_file.`);
        throw err;
      }
      ...
      for (const e of entries) {
        ...
        if (relToRoot && !relToRoot.startsWith('..') && ig.ignores(isDir ? `${relToRoot}/` : relToRoot)) continue;
        total++;
        ...
        if (isDir && level < depth) await walk(full, level + 1, `${prefix}  `);
      }
    };
    ...
    if (!out.length) return { content: `${rel} is empty.`, summary: 'empty' };

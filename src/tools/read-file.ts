...
export const readFileTool: Tool = {
  name: 'read_file',
  description:
    'Read a text file with line numbers. Use offset/limit for large files. You MUST read a file before editing it with edit_file.',
  permission: 'read',
  parameters: {
    type: 'object',
    properties: {
  ...
  async execute(args: ToolArgs, ctx: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      if (code === 'ENOENT') return { content: `File not found: ${rel}. Paths are relative to the working directory; use list_dir or glob to find the correct path.`, isError: true };
      if (code === 'EISDIR') return { content: `${rel} is a directory. Use list_dir to see its contents.`, isError: true };
      return { content: `Cannot read ${rel}: ${(err as Error).message}`, isError: true };
    }
    if (isProbablyBinary(buf)) return { content: `${rel} appears to be a binary file (${buf.length} bytes); not shown.`, isError: true };
    ...
    if (total === 0) return { content: `${rel} is empty.`, summary: 'empty' };
    if (start >= total) return { content: `Offset ${offset} is beyond the end of file (${total} lines).`, isError: true };

...
// `ignore` is CommonJS; under NodeNext the default import is module.exports (the factory),
// but its typings describe a namespace with a `default` member. Resolve both at runtime.
type IgnoreFactory = typeof ignoreImport.default;
const ignore: IgnoreFactory =
  (ignoreImport as unknown as { default?: IgnoreFactory }).default ?? (ignoreImport as unknown as IgnoreFactory);

export function resolvePath(cwd: string, p: string): string {
  const cleaned = (p ?? '').trim().replace(/^["']|["']$/g, '');
  if (!cleaned || cleaned === '.') return path.resolve(cwd);
...
export function reqString(args: ToolArgs, key: string, aliases: string[] = []): string {
...
export function optString(args: ToolArgs, key: string, aliases: string[] = []): string | undefined {
  for (const k of [key, ...aliases]) {
    const v = args[k];
    if (typeof v === 'string') return v;
  }
...
export function optNumber(args: ToolArgs, key: string, aliases: string[] = []): number | undefined {
  for (const k of [key, ...aliases]) {
    const v = args[k];
...
export function truncateText(text: string, maxChars: number, maxLines = 500): { text: string; truncated: boolean; droppedLines: number } {
  const lines = text.split('\n');
  const tooLong = text.length > maxChars || lines.length > maxLines;
  if (!tooLong) return { text, truncated: false, droppedLines: 0 };
...
export function loadIgnore(cwd: string): Ignore {
  const ig = ignore();
...
export function tmpDir(cwd: string): string {

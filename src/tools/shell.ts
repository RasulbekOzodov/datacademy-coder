...
function onPath(exe: string): boolean {
  if (found.has(exe)) return found.get(exe)!;
 ...
 */
function shellStarts(exe: string, args: string[], timeoutMs = 8000): Promise<boolean> {
...
export let shellFallbackNote: string | undefined;

export async function resolveShell(config: Pick<Config, 'shell'>): Promise<ResolvedShell> {
  let kind: ShellKind;
  if (config.shell === 'auto') {
    if (process.platform === 'win32') {
      if (onPath('pwsh') && (await shellStarts('pwsh', ['-NoProfile', '-NonInteractive', '-Command', 'echo ok']))) kind = 'pwsh';
      else if (await shellStarts('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', 'echo ok'])) kind = 'powershell';
      else {
        kind = 'cmd';
  ...
  } else kind = config.shell;

  return shellFor(kind);
}

export function shellFor(kind: ShellKind): ResolvedShell {
  switch (kind) {
    case 'pwsh':
    case 'powershell': {
...
export function runShell(command: string, opts: { cwd: string; shell: ResolvedShell; timeoutMs: number; signal?: AbortSignal; maxOutput?: number }): Promise<ShellRunResult> {
  ...
  return new Promise((resolve) => {
    ...
    const onData = (d: Buffer) => {
      if (size < cap) {
        ...
        chunks.push(d);
        size += d.length;
        while (size > cap * 1.5 && chunks.length > 1) size -= chunks.shift()!.length;
      }
    };
...
export function makeShellTool(shell: ResolvedShell): Tool {
  return {
    name: 'shell',
    description: `Run a ${shell.displayName} command in the working directory and return its output and exit code. Not interactive. ${
      shell.kind === 'powershell' ? 'Chain commands with ";" (not "&&").' : shell.kind === 'cmd' ? 'Use cmd.exe syntax.' : ''
    }`.trim(),
    permission: 'exec',
    parameters: {
      type: 'object',

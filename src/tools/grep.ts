...
function runRg(args: string[], cwd: string, signal?: AbortSignal): Promise<{ code: number | null; out: string }> {
...
export const grepTool: Tool = {
  ...
  async execute(args: ToolArgs, ctx: ToolContext): Promise<ToolResult> {
    ...
    if (await hasRg()) {
      ...
      for (const d of ALWAYS_IGNORE) rgArgs.push('--glob', `!${d}`);
      rgArgs.push(target);
      try {
        const { code, out } = await runRg(rgArgs, ctx.cwd, ctx.signal);
        if (code === 2) return { content: `rg error: ${out.slice(0, 500)}`, isError: true };
    ...
    try {
      ...
      return { content: `Cannot search ${relPath(ctx.cwd, target)}: ${(err as Error).message}`, isError: true };
    }
    ...
    for (const abs of files) {
      ...
      for (const line of text.split(/\r?\n/)) {
        ...
        if (re.test(line)) {
          ...
          if (++perFile >= 50 || lines.length >= max) break;
        }
      }
    }
    return formatMatches(lines, max, ctx.cwd);
...
function formatMatches(lines: string[], max: number, cwd: string): ToolResult {
  ...
  if (!norm.length) return { content: 'No matches.', summary: '0 matches' };

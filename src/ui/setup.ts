...
const LOCAL_MODEL_CHOICES: Array<{ id: string; note: string }> = [
...
async function yesNo(reader: LineReader, prompt: string, def = true): Promise<boolean | null> {
...
async function ollamaModels(): Promise<string[] | undefined> {
  try {
    const tags = await fetchJson<{ models?: Array<{ name: string }> }>(`${OLLAMA_URL}/api/tags`, { timeoutMs: 3000 });
...
async function installOllamaWindows(): Promise<number> {
  ...
  try {
    ...
    out(pc.red(`  yuklab bo'lmadi: ${(err as Error).message}\n`));
    return 1;
  }
...
function run(cmd: string, args: string[], opts: { shell?: boolean } = {}): Promise<number> {
...
async function waitForOllama(seconds: number): Promise<boolean> {
  for (let i = 0; i < seconds; i++) {
    if (await ollamaModels()) return true;
...
async function ensureOllama(reader: LineReader): Promise<boolean> {
  if (await ollamaModels()) return true;
  ...
  if (!ollamaInstalled()) {
    ...
    if (process.platform === 'win32') {
      if (hasCommand('winget')) {
        ...
        if (code !== 0) out(pc.dim(`  winget xato kodi ${code} — rasmiy installer yuklanadi\n`));
      } else out(pc.dim('  winget topilmadi — rasmiy installer yuklanadi\n'));
      if (code !== 0 || !ollamaInstalled()) code = await installOllamaWindows();
    } else if (process.platform === 'darwin') {
      code = hasCommand('brew') ? await run('brew', ['install', 'ollama']) : 127;
      if (code === 127) out(pc.yellow('  Homebrew yo\'q. Ollama\'ni https://ollama.com/download dan o\'rnating (Ollama.app), keyin qayta: datacademy_coder --setup\n'));
    } else code = await run('sh', ['-c', 'curl -fsSL https://ollama.com/install.sh | sh']);
  ...
  if (await waitForOllama(30)) return true;
...
async function ensureModel(reader: LineReader, model: string): Promise<boolean> {
  const models = (await ollamaModels()) ?? [];
  if (models.some((m) => m === model || m.startsWith(`${model}:`))) return true;
...
export async function runSetupWizard(reader: LineReader): Promise<boolean> {
  ...
  // ---- local
  if (!(await ensureOllama(reader))) return false;
  ...
  if (installed.length) {
    ...
    if (n <= installed.length) model = installed[n - 1];
    else {
  ...
  if (!(await ensureModel(reader, model))) return false;
...
async function chooseModelToPull(reader: LineReader): Promise<string | null> {
  ...
  if (n <= LOCAL_MODEL_CHOICES.length) return LOCAL_MODEL_CHOICES[n - 1].id;

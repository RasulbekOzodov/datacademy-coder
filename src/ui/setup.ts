...
async function ensureOllama(reader: LineReader): Promise<boolean> {
  ...
  if (!ollamaInstalled()) {
    ...
    let code = 1;
    if (process.platform === 'win32') code = await run('winget', ['install', '--id', 'Ollama.Ollama', '-e', '--silent', '--accept-source-agreements', '--accept-package-agreements']);
    else if (process.platform === 'darwin') code = await run('brew', ['install', 'ollama']);
